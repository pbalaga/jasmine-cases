...
function createVariantDescription(description, args, index) {
    var variantDesc = description + " (Variant #" + index + " <", i = 0, length = args.length, x;
    for (i; i < length; i++) {
        if (i > 0) {
            variantDesc += ", ";
        }
...
function createDataDrivenSpecs(specProvider, description, dataset, fn, isAsyncAllowed) {
    var i = 0, length = 0, specs = [], args, maxArgCount = 0, variantDesc, suite;
    ...
    for (length = dataset.length; i < length; i++) {
        ...
        if (args.length !== maxArgCount) {
            throw error("Jasmine.ArgumentCountMismatchError", "Expected {0} argument(s). Found {1} at index {2} ({3})", maxArgCount, args.length, i, description);
        }
            ...
            throw error("Jasmine.ArgumentCountMismatchError", "Expecting data driven spec to accept {0} {1}, but {2} {3} specified in the callback function ({4})", args.length, args.length === 1 ? "argument" : "arguments", fn.length, fn.length === 1 ? "argument is" : "arguments are", description);
        }
    }
...
//# sourceMappingURL=C:/Projects/jasmine-cases/all.js.map